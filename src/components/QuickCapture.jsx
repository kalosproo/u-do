...
export default function QuickCapture() {
  ...
  const reset = useCallback(() => { setNote(""); setEntry(null); setStatus(""); }, []);
  const open = useCallback(() => { lastFocusRef.current = document.activeElement; setIsOpen(true); }, []);
  const close = useCallback(() => { setIsOpen(false); reset(); }, [reset]);

  useEffect(() => {
    const openCapture = () => open();
    window.addEventListener("udo:quick-capture", openCapture);
    return () => window.removeEventListener("udo:quick-capture", openCapture);
  }, [open]);
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("keydown", onKeyDown); lastFocusRef.current?.focus?.(); };
  }, [close, isOpen]);
  ...
  return <>
    <button type="button" className="quickcap-fab" onClick={open} aria-label="Quick capture" title="Quick capture"><FiZap /></button>
    {isOpen && createPortal(<div className="quickcap-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}><section className="quickcap-popup" role="dialog" aria-modal="true" aria-labelledby="quickcap-title"><div className="quickcap-header"><h4 id="quickcap-title">Quick Capture</h4><button type="button" className="quickcap-close" onClick={close} aria-label="Close"><FiX /></button></div><p className="quickcap-muted">Type it like you'd say it — “spent 20 on juice”, “call mom tomorrow”, “meditate daily”.</p>
      {!entry ? <><input ref={noteRef} className="quickcap-input" aria-label="What do you need to capture?" placeholder="What do you need to do?" value={note} onChange={(event) => setNote(event.target.value)} onKeyDown={(event) => event.key === "Enter" && capture()} /><button type="button" className="quickcap-primary" onClick={capture} disabled={loading || !note.trim()}>{loading ? "Reading..." : "Capture"}</button></> : <div className="quickcap-confirm"><div className="quickcap-type-row">{TYPE_OPTIONS.map((type) => <button key={type} type="button" className={`quickcap-type-pill ${entry.type === type ? "active" : ""}`} onClick={() => setEntry((current) => ({ ...current, type }))}>{type}</button>)}</div><input className="quickcap-input" placeholder="Title" value={entry.title} onChange={(event) => setEntry((current) => ({ ...current, title: event.target.value }))} />
